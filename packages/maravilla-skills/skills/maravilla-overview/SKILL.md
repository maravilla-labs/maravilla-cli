...
10. **`STORAGE.get` returns a `Uint8Array`** in current runtime builds. Older builds returned `Array<number>` (op was `#[serde]`-tagged); if your app may run against a mixed pool, keep the defensive coercion `obj instanceof Uint8Array ? obj : Array.isArray(obj) ? new Uint8Array(obj) : new Uint8Array(await new Response(obj).arrayBuffer())` until your fleet is on the post-fix runtime.
